...
export async function createUser(db: pg.Pool, email: string, password: string): Promise<UserRow> {
  ...
  return { id, email: email.toLowerCase().trim(), password_hash: passwordHash };
}
...
export async function listProjectsForUser(db: pg.Pool, userId: string): Promise<ProjectRow[]> {
  const { rows } = await db.query<ProjectRow>(
    `SELECT id, user_id, name, company_id, created_at::text FROM projects WHERE user_id = $1 ORDER BY created_at DESC`,
    [userId],
  );
  return rows;
}

export async function createProject(db: pg.Pool, userId: string, name: string): Promise<ProjectRow> {
  const id = crypto.randomUUID();
  ...
  const { rows } = await db.query<ProjectRow>(
    `SELECT id, user_id, name, company_id, created_at::text FROM projects WHERE id = $1`,
    [id],
  );
  return rows[0]!;
}
...
export async function getProjectForUser(
  ...
  const { rows } = await db.query<ProjectRow>(
    `SELECT id, user_id, name, company_id, created_at::text FROM projects WHERE id = $1 AND user_id = $2`,
    [projectId, userId],
  );
  return rows[0];
}
