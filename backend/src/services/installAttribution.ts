...
export function resolveInstallAttribution(
  ...
  const settings = getAttributionSettings(customerDb, event.companyId);
  const salt = tokenSaltForCompany(event.companyId);
  const tokenHash = hashToken(event.token, salt);
  const identity = getIdentityForSession(deviceDb, event.companyId, sessionId);

  const candidate = runMatchRules(customerDb, deviceDb, {
  ...
  if (!candidate) {
    checkFraudFlags(deviceDb, event.companyId, event.eventId, { type: "organic_install" });
  ...
  if (!clickEventId && candidate.pendingClick) {
    ...
    if (!existing) {
      storeClick(customerDb, pendingClickToRecordEvent(candidate.pendingClick, event.companyId, event.token));
    }
    clickEventId = candidate.pendingClick.click_id;
    if (identity?.identityId) {
      markPendingClickMatched(deviceDb, candidate.pendingClick.click_id, identity.identityId);
    }
  }
  ...
  checkFraudFlags(deviceDb, event.companyId, event.eventId, {
