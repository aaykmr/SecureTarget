...
4. Use the same **`API_KEY_PEPPER`** and **`EVENTIQN_DB_PATH`** (or equivalent DB path) for both the dashboard and ingest server so keys hash consistently.
...
## Backend endpoints
- `POST /v1/session/bootstrap` — **once per browser/app install** (or after `clearSession`). Body `{ occurredAt, device }` must include `device.platform` (`web` | `ios` | `android`). Device details are persisted in the EventIQN device DB for install attribution matching; customer DB stores only opaque `sessionId`.
...
After the script loads, use the global **`EventIQN`** object (same API as `init` from npm):

```html
<script src="https://your-dashboard.example.com/sdk.js" async></script>
<script>
  document.addEventListener("DOMContentLoaded", function () {
    var st = EventIQN.init({
      apiKey: "YOUR_API_KEY",
...
```ts
import { init } from "@eventiqn/web-sdk";
...
- Optional: call `bootstrapSession(device:)` early with your own `EventIQNDeviceDetails`.
...
- Device details (advertising IDs, install referrer, IP at bootstrap) are stored in the separate EventIQN device DB (`EVENTIQN_DEVICE_DB_PATH`) for install matching.
