...
import type { ApiKeyRow } from "@/lib/repos";
...
function KeyRow({
  ...
  return (
    <li className={clsx(styles.keyRow, subdued && styles.keyRowSubdued)}>
      ...
      {showRevoke && !revoked ? <RevokeKeyForm projectId={projectId} keyId={keyRow.id} /> : null}
    </li>
  );
}

export function ApiKeyList({ keys, projectId }: { keys: ApiKeyRow[]; projectId: string }) {
    ...
    <div className={styles.root}>
      {activeKey ? (
        <ul className={styles.keyList}>
          <KeyRow keyRow={activeKey} projectId={projectId} showRevoke />
        </ul>
      ) : (
        ...
        <details className={styles.rollup} open={legacyOpen} onToggle={(e) => setLegacyOpen(e.currentTarget.open)}>
          ...
          <ul className={styles.keyList}>
            {legacyActiveKeys.map((k) => (
              <KeyRow key={k.id} keyRow={k} projectId={projectId} showRevoke subdued />
            ))}
          </ul>
        </details>
      ) : null}
